...
export class PackageBundler {
  ...
  /** Bundles the primary entry-point w/ given entry file, e.g. @metaclinic/cdk */
  ...
  /** Bundles a single secondary entry-point w/ given entry file, e.g. @metaclinic/cdk/a11y */
  ...
  private async createRollupBundle(config: RollupBundleConfig) {
    ...
    if (config.format === 'umd') {
      ...
      if (this.buildPackage.exportsSecondaryEntryPointsAtRoot &&
        ...
        // Use the rollup-alias plugin to map imports of the form `@metaclinic/material/button`
  ...
  /**
   * Gets mapping of import aliases (e.g. `@metaclinic/material/button`) to the path of the es5
   * bundle output.
