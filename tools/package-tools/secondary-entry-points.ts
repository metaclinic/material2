...
export function getSecondaryEntryPointsForPackage(pkg: BuildPackage) {
  ...
  // E.g., extract "portal" from "from '@metaclinic/cdk/portal';".
...
function buildPackageImportStatementFindCommand(searchDirectory: string, packageName: string) {
  if (platform() === 'win32') {
    return {
      binary: 'findstr',
      args: ['/r', `from.'@metaclinic/${packageName}/.*'`, `${searchDirectory}\\*.ts`]
    };
  } else {
    return {
      binary: 'grep',
      args: ['-Eroh', '--include', '*.ts', `from '@metaclinic/${packageName}/.+';`, searchDirectory]
    };
  }
}
