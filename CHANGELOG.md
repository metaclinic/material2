...
* All cdk re-exports in `@metaclinic/material` have been removed. See the [the beta.10 notes for more
...
available from @metaclinic/material and instead must be imported from @metaclinic/cdk/collections
...
### Highlights
* Each `@metaclinic/material` component is now bundled into its own javascript file. This will allow
tools like webpack to more easily load _only_ the components being used in an application.
* New stepper component! The base behavior lives in `@metaclinic/cdk` with Material Design flavors in
`@metaclinic/material`.
...
* `MdCoreModule` has been removed. Most of its functionality has been moved to `@metaclinic/cdk` over
the last few releases.
* `FocusOriginMonitor` has been renamed to `FocusMonitor` and moved to `@metaclinic/cdk`.
...
to `FocusMonitor` and moved to `@metaclinic/cdk/a11y` (`A11yModule`).
